...
| `test_content.py` | `zpf.content` — the `content_type` grammar, the `prim:` decode, and the `ContentRegistry` lookups — plus `Record.content()`. (`FileReader.content()`, which needs a file to resolve `dec:`, lives in `test_reader.py`.) |
